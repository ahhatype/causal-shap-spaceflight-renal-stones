...
- `apps/`: Python Shiny companion app, self-contained deterministic bundles,
...
python -m unittest discover -s apps/tests -v
cd apps
...
`apps/bundles/analysis_output_baseline_hashes.json`. This preserves the gate
...
python -m unittest discover -s apps/tests -v
python -m compileall -q apps/causal_shap apps/stages apps/tests apps/app.py
...
```bash
quarto render site
.venv/bin/python -m unittest discover -s apps/tests -v
