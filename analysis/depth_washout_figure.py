...
regression of Y on X_d is tested at alpha = 0.05. In a chain with no
...
def draw(curves: pd.DataFrame) -> plt.Figure:
    ...
    right.set_xlabel("sampling standard error  (1 / √n, larger = smaller sample)", color=INK, fontsize=9.5)
    right.set_ylabel("chance the node is detected at all", color=INK, fontsize=9.5)
    ...
    fig.text(
        0.005, 0.01,
        f"Standardized linear chain, every edge {BETA}; {REPLICATES} replicates per point; alpha 0.05 on the marginal slope of Y on the node. Seed {SEED}.",
