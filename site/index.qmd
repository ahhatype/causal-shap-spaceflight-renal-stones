...
::: {.editorial-shell}
::: {.hero}
<p class="lineage-tag"><a href="https://www.tao-rwd.com/acic-2026/causal-shap">ACIC 2026 CAUSAL SHAP</a> <span>→</span> <a href="https://andystats.github.io/causal-shap-target-dags/">TARGET DAGS</a> <span>→</span> SPACEFLIGHT RENAL STONES</p>

# Prediction is the default playbook. **Intervention casts a wider net.**

<p class="hero-lede">A predictor can ignore an upstream cause the moment its mediator is in view. An intervention cannot. This hub tests, on NASA's renal-stone topology with an answer key we wrote, <mark>how to re-admit the deeper nodes the prediction playbook washes out</mark>, and how to prune them without fooling ourselves.</p>

Two goals, two recipes. The recipe for prediction is well known and works. The recipe for intervention needs a detector for depth, a filter for noise, and a structural model for pruning. Everything here is synthetic, source-aligned, and scored against frozen total-effect truth.
...
<p class="section-deck">Harrell's <em>Regression Modeling Strategies</em> separates hypothesis testing, effect estimation, and prediction on its first page and gives each its own recipe. Explainable AI runs the prediction recipe by default. That is fine for prediction and structurally wrong for intervention.</p>
...
## The intervention playbook {#pipeline}

<p class="section-deck">Each rung inherits the one before it. The prediction playbook stops at the second. Current evidence reaches the fifth; the third and fourth are the placeholders this hub is built to fill.</p>
...
A learned-kernel classifier (Lucidity Sciences) exposes a per-feature
design vector. Reduced per feature, it gives a signal that on one renal run
